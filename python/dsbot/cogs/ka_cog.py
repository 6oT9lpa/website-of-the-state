...
class ka_cog(commands.Cog):
    ...
    async def add_department_otrab(self, inter: disnake.ApplicationCommandInteraction, department: str, warning_type: warn_variants, otrabotka: str):
        ROLE_ID = 1150717914169753722
        if disnake.utils.get(inter.author.roles, id=ROLE_ID):
            config = load_config()
            server_id = str(inter.guild.id)
    ...
    async def warn(inter: disnake.ApplicationCommandInteraction, member: disnake.Member, warning_type: warn_variants, reason_type: str):
        ...
        if disnake.utils.get(inter.author.roles, id=ROLE_ID):
            ...
            # Находим департамент, связанный с участником (например, SWAT)
            ...
            if user_department:
                ...
                if user:
                    ...
                    if warning_type.lower() == 'ustnik':
                            ...
                            embed = disnake.Embed(
                            title="Кадровый аудит • Устный выговор",
                            ...
                            embed.add_field(name="Итого:", value=f"{user.YW}/2, {user.SW}/2", inline=True)
                    elif warning_type.lower() == 'strogaj':
                            if user.SW == "1":
                                warn_role_list = warn_roles.get("strogaj2", [])
                                if warn_role_list:
                                ...
                                user.SW = "1"
                            embed = disnake.Embed(
                            title="Кадровый аудит • Строгий выговор",
        ...
        if disnake.utils.get(inter.author.roles, id=ROLE_ID):
                ...
                # Обновление полей YW и SW в зависимости от типа выговора
                        if warning_type.lower() == 'ustnik':
                            ...
                            user.YW = "0"
                            embed = disnake.Embed(
                            title="Кадровый аудит • Устный снят",
                            ...
                            if user.SW == "1":
                                ...
                                user.SW = 1
                            embed = disnake.Embed(
                            title="Кадровый аудит • Строгий снят",
                            color=disnake.Color.green()
                            )
                            embed.add_field(name="Снял:", value=f"<@{inter.author.id}>", inline=True)
                            embed.add_field(name="Кому:", value=f"<@{member.id}>", inline=True)
...
def setup(bot):
    ...
    #bot.loop.create_task(send_audit_message())